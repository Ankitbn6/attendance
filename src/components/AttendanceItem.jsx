...
const AttendanceItem = ({ele,list,count,setCount,storeLocally}) => {
    ...
    const clickedStyle=clicked?{backgroundColor:"rgb(255,180,0)"}:{}
    const setTime=(ele)=>{
        if(!clicked)
        {
          let date=new Date();
  ...
  return (
    <div onClick={setTime} style={clickedStyle} className='bg-[rgb(255,230,0)] m-auto text-center flex items-center  justify-between px-3 gap-1 text-lg py-2 my-1 '>
      ...
      <input className='w-[35px] p-1 bg-transparent text-2xl outline-none' value={currentHour} onChange={(e)=>{setCurrentHour(e.target.value)}} />:
